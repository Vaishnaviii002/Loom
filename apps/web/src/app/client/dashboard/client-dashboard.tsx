...
function DashboardView({
  ...
  const prdContent = [
    ...
    `Requested change`,
    `The requested change should be clearly implemented for the identified product area while keeping the final result client-verifiable and aligned with the original request.`,

    `Change impact`,
    `- This change may affect the current user experience and visible product behavior.
- It may require updates in the related UI section, content area, or connected user flow.
- The final implementation should remain consistent with the existing product design and functionality.`,

    `Acceptance criteria`,
    `- The change is clearly scoped and implementation-ready.
- The expected result is visible and understandable to the client.
- The final output can be reviewed by the product team before engineering execution.
- The client can verify the delivered change without seeing internal code or technical review details.`,

    `Edge cases`,
    `- The request may still need minor clarification before final implementation.
- The requested functionality may partially already exist and may only need refinement.
- The change may affect responsiveness, layout consistency, or related screens.`,

    `Success metrics`,
    `- The PRD clearly explains what must be delivered.
- The team can convert the PRD into execution work without ambiguity.
- The client can understand the expected final outcome.
- The delivered work can be checked directly against the acceptance criteria.`,
