...
export default function SeniorReviewClient({
  ...
  const [activeView, setActiveView] = useState<
    "summary" | "pull-requests" | "final-approval"
  >("summary");

  const queueSummaries = useMemo(() => {
    const latestByTask = new Map<string, SeniorSummaryItem>();
  ...
  const [selectedId, setSelectedId] = useState(
    selectedSummaryId || queueSummaries[0]?.id || "",
  );
  ...
  const selectedSummary = useMemo(() => {
    return (
      queueSummaries.find((summary) => summary.id === selectedId) ||
      queueSummaries[0]
    );
  }, [queueSummaries, selectedId]);
  ...
  return (
    <main className="h-screen overflow-hidden bg-[#111111] text-white">
      <div className="grid h-screen lg:grid-cols-[20%_80%]">
        ...
        <section className="flex h-screen min-h-0 flex-col overflow-hidden px-10 py-10">
          ...
          {queueSummaries.length === 0 ? (
            ...
            <div className="mt-10 grid min-h-0 flex-1 gap-6 xl:grid-cols-[30%_70%]">
              <section className="h-fit max-h-full overflow-y-auto pr-2 xl:sticky xl:top-0">
                  ...
                  {queueSummaries.map((summary) => {
                    ...
                    return (
                      <button
                        ...
                        className={
                          selectedSummary?.id === summary.id
                            ? "w-full rounded-2xl border border-[#aa4825]/50 bg-white/[0.04]10 p-4 text-left"
                      ...
                      >
                        ...
                        <p className="mt-2 text-xs text-[#eae3e0]">
                        ...
                        decision.status !== "REJECTED" ? (
                          <span className="mt-3 inline-flex rounded-full border border-emerald-500/30 bg-emerald-500/10 px-3 py-1 text-[11px] font-semibold uppercase tracking-wide text-emerald-200">
                            Merged to main
                          </span>
                        ) : null}
                      </button>
...
function FinalApprovalReadOnly({
  ...
  async function submitSeniorDecision(
    decisionType: "REQUEST_CHANGES" | "REJECT",
  ) {
    if (!summary) return;
  ...
  const pullRequestUrl =
    summary.pullRequestUrl || summary.summary.pullRequestSummary.pullRequestUrl;
